...
class ReportManager{
	 ...
	 *
	 */
	public function getReportsOf(string $name, $reportTypes = null) : array{
		...
		foreach($this->reports as $report){
			...
			if($report instanceof HackReport && $report->getReported() === $name){
				$result[$report->getTimestamp()] = $report;
			}elseif($report instanceof StaffReport && $report->getReported() === $name){
				$result[$report->getTimestamp()] = $report;
			}
		}
	 ...
	 * @return array|ReportInfo[]
	 */
	public function getReportHistoryOf(string $name) : array{
	 ...
	 * Gets the report.
	 */
	public function getReport(string $localName) : ?ReportInfo{
	...
	public function sendTranslatedReport(MineceitPlayer $player, ReportInfo $info, bool $history) : void{
		...
		$lang = $player->getLanguageInfo()->getLanguage()->getTranslationName(TranslateUtil::GOOGLE_TRANSLATE);
