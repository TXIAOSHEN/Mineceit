...
class PlayerReplayData extends ReplayData{
	...
	/* @var array|Vector3[]
	 * Current Motions of the player at a given time.
	 */
	private $motionTimes;
	...
	/* @var array|float[]
	 * Determines when the player is teleporting.
	 */
	private $tpTimes;
	 ...
	 * @param Vector3 $pos
	 */
	public function setTeleportAt(int $tick, Vector3 $pos) : void{
		$this->tpTimes[$tick] = $pos;
	}
	...
	public function setPickupAt(int $tick, Item $item) : void{
		$droppedItem = null;
		$currentTick = $tick;
		while($droppedItem === null && $currentTick >= 0){
	...
	public function setItemAt(int $tick, Item $item) : void{
		...
		if($item->getId() !== $lastValue->getId() || ($item->getId() === $lastValue->getId() && $item->getDamage() !== $lastValue->getDamage()))
			$this->itemTimes[$tick] = $item;
	}
	 ...
	 */
	public function setRotationAt(int $tick, array $rotation) : void{
	...
	public function setEffectAt(int $tick, EffectInstance $effect) : void{
		...
		while($startTick <= $length){
			if($this->deathTime !== -1 && $startTick >= $this->deathTime)
				return;
	...
	public function updateArmor(int $tick, array $armor = []) : void{
		...
		foreach($keys as $key){
			...
			}else if(isset($lastArmorUpdate[$key]) && isset($armor[$key])){
				/* @var Item $lastArmor */
	 ...
	 * @param Vector3 $vec3
	 */
	public function setMotionAt(int $tick, Vector3 $vec3) : void{
		$this->motionTimes[$tick] = $vec3;
	}
	...
	public function getAttributesAt(int $tick) : array{
		...
		if(isset($this->tpTimes[$tick]))
			$result['tp'] = $this->tpTimes[$tick];
		...
		if(isset($this->motionTimes[$tick]))
			$result['motion'] = $this->motionTimes[$tick];
	...
	public function getLastAttributeUpdate(int $tick, string $attribute){
		...
		switch($attribute){
			...
			case 'position':
				...
				break;
			case 'motion':
				$searchedArray = $this->motionTimes;
				break;
			case 'fire':
