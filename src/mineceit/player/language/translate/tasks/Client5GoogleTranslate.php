...
class Client5GoogleTranslate extends AsyncTask{
	...
	public function __construct(MineceitPlayer $player, $msg, string $originalLang = "", string $format = "", int $transContMsgParam = 0){
		...
		$translatedLang = $player->getLanguageInfo()
	...
	public function onCompletion(Server $server){
		...
		if($core instanceof MineceitCore && $core->isEnabled() && $result !== null){
			...
			if(($player = MineceitUtil::getPlayer($this->playerName)) !== null && $player instanceof MineceitPlayer){

				$translateLang = $player->getLanguageInfo()
					->getLanguage()->getTranslationName(TranslateUtil::GOOGLE_TRANSLATE);

				$result = $translated;
				if(strtolower($translated) === strtolower($original) || $translated === '' || $outputOriginalLang === $translateLang){
					$result = $original;
				}
