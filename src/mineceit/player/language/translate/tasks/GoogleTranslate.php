...
class GoogleTranslate extends AsyncTask{
	...
	public function __construct(MineceitPlayer $player, string $msg, string $originalLang = "", string $format = ""){
		...
		$translatedLang = $player->getLanguageInfo()->getLanguage()
			->getTranslationName(TranslateUtil::GOOGLE_TRANSLATE);
		$this->format = $format;
	...
	public function onCompletion(Server $server){
		...
		if($core instanceof MineceitCore && $core->isEnabled() && $result !== null){
			...
			if(($player = MineceitUtil::getPlayerExact($this->playerName)) !== null && $player instanceof MineceitPlayer){

				$translateLang = $player->getLanguageInfo()
					->getLanguage()->getLocale();

				$result = $translated;
				if(strtolower($translated) === strtolower($original) || $translated === '' || $originalLang === $translateLang){
					$result = $original;
				}
