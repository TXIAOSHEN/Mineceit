...
class StatsHologram extends LeaderboardHologram{
	...
	protected function placeFloatingHologram(bool $updateKey = true) : void{
		...
		if($size > 0){
			...
			$title = TextFormat::LIGHT_PURPLE . TextFormat::BOLD . $keyName . TextFormat::WHITE . ' Leaderboards';
			foreach($text as $name => $stat){
				if($count > 9) break;
				$line = $count === 9 ? "" : "\n";
				$stat = $text[$name];
				$place = $count + 1;
				$format = TextFormat::GRAY . $place . '. ' . TextFormat::LIGHT_PURPLE . $name .
					' ' . TextFormat::DARK_GRAY . '(' . TextFormat::WHITE . $stat . TextFormat::DARK_GRAY . ')' . $line;
				$string .= $format;
				$count++;
			}
